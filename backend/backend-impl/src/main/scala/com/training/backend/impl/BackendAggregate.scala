...
/**
  * This provides an event sourced behavior. It has a state, [[BackendState]], which
  ...
  */
object BackendBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[BackendCommand]): Behavior[BackendCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, BackendEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[BackendCommand, BackendEvent, BackendState](
        persistenceId = persistenceId,
        emptyState = BackendState.initial,
...
/**
  ...
  */
case class BackendState(message: String, timestamp: String) {
  def applyCommand(cmd: BackendCommand): ReplyEffect[BackendEvent, BackendState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: BackendEvent): BackendState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[BackendEvent, BackendState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[BackendEvent, BackendState] =
    Effect
...
object BackendState {
  ...
  /**
    ...
    */
  def initial: BackendState = BackendState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[BackendCommand]("BackendAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[BackendState] = Json.format
}

/**
  * This interface defines all the events that the BackendAggregate supports.
  */
sealed trait BackendEvent extends AggregateEvent[BackendEvent] {
  def aggregateTag: AggregateEventTag[BackendEvent] = BackendEvent.Tag
}

object BackendEvent {
  val Tag: AggregateEventTag[BackendEvent] = AggregateEventTag[BackendEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends BackendEvent
...
/**
  ...
  */
trait BackendCommandSerializable

/**
  * This interface defines all the commands that the BackendAggregate supports.
  */
sealed trait BackendCommand
    extends BackendCommandSerializable
...
/**
  ...
  * application loader.
  */
object BackendSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[BackendState],
